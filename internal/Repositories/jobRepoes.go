...
func (r *JobRepo) GetActiveJobs() ([]models.Job, error) {
	...
	Preload("Business").
	Preload("Category").
	Where("is_active = ? AND status = ?", true, "open").
	Find(&jobs).Error
	return jobs, err
}
...
func (r *JobRepo) GetJobsByCategory(categoryID uint) ([]models.Job, error) {
	...
	Preload("Business").
	Preload("Category").
	Where("category_id = ? AND is_active = ? AND status = ?", categoryID, true, "open").
	Find(&jobs).Error
	return jobs, err
}
...
func (r *JobRepo) GetJobsByLocality(locality string) ([]models.Job, error) {
	...
	Where("businesses.locality = ? AND jobs.is_active = ? AND jobs.status = ?", locality, true, "open").
	Find(&jobs).Error
	return jobs, err
}
...
func (r *JobRepo) UpdateJobWithDescription(job *models.Job, desc *models.JobDescription) error {
	return r.DB.Transaction(func(tx *gorm.DB) error {
		...
		desc.ID = existing.ID
		desc.JobID = job.ID
		return tx.Save(desc).Error
	})
}

func (r *JobRepo) UpdateJobStatus(jobID uint, isActive bool, status string) error {
