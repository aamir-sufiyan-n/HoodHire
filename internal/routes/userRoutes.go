...
func SetupRoutes(app *fiber.App, handler *app.APP) {
	configRepo := &repositories.WebRepo{DB: database.DB}
	...
	app.Get("/categories", handler.Cathandler.GetAllCategories)
	app.Get("/categories/stats", handler.Cathandler.GetCategoryStats)
	...
	seeker.Post("/send-otp",
		middlewares.FeatureMiddleware(configRepo, "user_registration"),
	...
	hirer.Post("/send-otp",
		middlewares.FeatureMiddleware(configRepo, "business_registration"),
		func(c fiber.Ctx) error {
			c.Locals("role", "hirer")
			return handler.AuthHandler.SendOTP(c)
		},
	...
	// ─── SEEKER ───────────────────────────────────────────
	seekerApi := app.Group("/seeker",
		middlewares.AuthMiddleware,
		middlewares.RoleMiddleware("seeker"),
	...
	seekerApi.Patch("/resume", handler.SeekerHandler.UploadResume)
	seekerApi.Delete("/resume", handler.SeekerHandler.DeleteResume)
	...
	seekerApi.Post("/jobs/:id/apply",
		middlewares.FeatureMiddleware(configRepo, "job_applying"),
		handler.JobHandlers.ApplyToJob,
	)
	...
	// ─── HIRER ────────────────────────────────────────────
	hirerApi := app.Group("/hirer",
		middlewares.AuthMiddleware,
		middlewares.RoleMiddleware("hirer"),
		middlewares.BlockMiddleware,
	)
	...
	hirerApi.Post("/jobs",
		middlewares.FeatureMiddleware(configRepo, "job_posting"),
		handler.JobHandlers.CreateJob,
	)
